...
/* --- blit_or ---
 * OR-composite blit: writes (src_word | dest_word) to each destination
 * word, saving the original destination word into `mask_save` first
 * (same save-then-combine structure as blit_masked/blit_transparent).
 * Used by level_objects.asm's draw_thrown_sprite (the falling-object
 * "prop" sprite) — a distinct third combine mode from AND (blit_masked)
 * and the chroma-key logic (blit_transparent), re-derived directly from
 * cga.asm's real "mov bx,[es:di] / mov [bp],bx / lodsw / or ax,bx /
 * stosw" loop. Pass NULL for mask_save if the caller doesn't need it. */
